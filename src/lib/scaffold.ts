...
async function copyTemplateFiles(
  ...
  if (!(await hasTemplate(language, basePath))) {
    console.log(`  ℹ️  No template found for '${language}' - AI will generate all files`);
    return false;
  }
...
export async function scaffoldChallenge(options: ScaffoldOptions): Promise<string> {
  ...
  for (const file of filteredSolutionFiles) {
    ...
    await fs.writeFile(filePath, file.content, 'utf-8');
    console.log(`  ✓ Created solution/${file.path}`);
  }
  ...
  for (const file of filteredTestFiles) {
    ...
    await ensureDirectoryExists(filePath);
    await fs.writeFile(filePath, file.content, 'utf-8');
    console.log(`  ✓ Created tests/${normalizedPath}`);
  }
  
  console.log('\n✅ Challenge scaffolded successfully!');
  console.log(`\n📂 Location: ${challengePath}`);
