...
import { CHALLENGE_GENERATOR_SYSTEM_PROMPT, buildUserPrompt } from '../lib/prompts.js';
...
export function createGenerateCommand(): Command {
  ...
  command
    ...
    .option('-l, --language <lang>', 'Programming language (typescript, php, python)', 'typescript')
    ...
    .action(async (options) => {
      try {
        ...
        if (!validLanguages.includes(language)) {
          console.error(`Error: Invalid language "${language}". Must be one of: ${validLanguages.join(', ')}`);
          process.exit(1);
        }
        ...
        console.log(`🎯 Generating Level ${difficulty} ${language} challenge...`);
        if (options.topic) {
        ...
        const challengeNumber = await getNextChallengeNumber(language, basePath);
        ...
        console.log(`  2. Start with the starter code: cd ${path.join(challengePath, 'starter')}`);
